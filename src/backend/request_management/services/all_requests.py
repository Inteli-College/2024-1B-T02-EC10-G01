...
async def fetch_all_requests_by_user(session: AsyncSession, user: dict):
    async with aiohttp.ClientSession() as http_session:
        # Query to fetch medicine requests
        medicine_query = select(
            MedicineRequest.id,
            MedicineRequest.dispenser_id,
            MedicineRequest.medicine_id,
            literal(None).label("assistance_type"),
            ...
            MedicineRequest.emergency,
            literal("medicine").label("request_type"),
        ).where(MedicineRequest.requested_by == user["id"])
        ...
        material_query = select(
            ...
            MaterialRequest.status,
            MaterialRequest.created_at,
            literal(None).label("batch_number"),
            literal(None).label("emergency"),
            literal("material").label("request_type"),
        ).where(MaterialRequest.requested_by == user["id"])

        # Query to fetch assistance requests
        assistance_query = select(
            AssistanceRequest.id,
            AssistanceRequest.dispenser_id,
            literal(None).label("material_id"),
            AssistanceRequest.assistance_type,
            AssistanceRequest.details,
            ...
            literal(None).label("emergency"),
            literal("assistance").label("request_type"),
        ).where(AssistanceRequest.requested_by == user["id"])
        ...
        for row in request_results:
            ...
            request_dict = {
                "id": row_dict['id'],
                "dispenser_id": row_dict['dispenser_id'],
                "material_id": row_dict.get('material_id'),
                "medicine_id": row_dict.get('medicine_id'),
                "assistance_type": row_dict['assistance_type'],
                "details": row_dict['details'],
                "created_at": row_dict['created_at'],
                "batch_number": row_dict['batch_number'],
                "emergency": row_dict['emergency'],
